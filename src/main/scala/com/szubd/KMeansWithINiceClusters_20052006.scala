...
object KMeansWithINiceClusters_20052006 {
  def main(args: Array[String]): Unit = {
    ...
    val outputPath = "data/Reddit20052006/results/mllib_kmeans_results5_5_100_1000/"
...
//      }.collect()
      ...
      val resultDF = df.withColumn("cluster_label", predictUDF($"svd_100d_features"))
      ...
      println(s"\n9. Saving cluster distribution: ")
      clusterStats.select("cluster_label", "cluster_size")
        .coalesce(1)
        .write
        .mode("overwrite")
        .option("header", "true")
        .csv(s"${outputPath}cluster_stats.csv")

      // Define UDF to get vector size (just to check if all vectors still have the same size)
//      val getVectorSize = udf((v: org.apache.spark.ml.linalg.Vector) => v.size)
//
//      // Add new column with vector size
//      val dfWithSize = clusterStats.withColumn("vector_size", getVectorSize(col("tfidf_mean")))
//      dfWithSize.show(10)
