...
//        }
        System.out.println("Creating RspDataset::");
        RspDataset<Row> svddataread = RspContext.SparkSessionFunc(spark).rspRead().csv("data/SVDdata1.csv");

        JavaRspRDD<List<double[]>> listJavaRspSVDRDD = apply_data(svddataread.javaRDD());
        ...
        //
        List<double[]> mergedCenters = I_niceMO_Observers.I_niceMOF(listJavaRspSVDRDD.rdd().flatMap(list -> list.iterator())  // 把每个 List<double[]> 打平
                .collect(),10,5,0.1);

        System.out.println("num of mergedCenters::"+mergedCenters.size());
...
//        });
        ...
        ScatterPlot plot = new ScatterPlot(datavector, mergedCenters,4, 15); // 红点 15px，蓝点 7px
        frame.add(plot);
        ...
        int[] labels = kmeans.fit(datavector);
