...
//            int randomNumber = (int) (Math.random() * result.size()) + 1; //随机选点
    ...
    // 在GO里面合并聚类中心
    public static List<double[]> mergeDataGOMO(List<double[]> centersAll,double percentage) {
...
//        double threshold = computeThreshold(distanceMatrix, percentage);  //手动设置阈值
    ...
    // 在GO里面合并聚类中心
    public static List<double[]> mergeDataGOSC(List<double[]> centersAll) {
        //谱聚类的GO方法
        // 计算相似度矩阵
...
//        }
//
//        KDTree kdTree = new KDTree(dataList.get(0).length);
//        for (double[] point : dataList) {
//            kdTree.insert(point, point);
//        }
//
//        double[][] distances = new double[n][k];
//        for (int i = 0; i < n; ++i) {
//            // 取 k+1 是为了排除自己
//            Object[] neighbors = kdTree.nearest(dataList.get(i), k + 1);
//            double[] dist = new double[k];
//            int index = 0;
//
//            for (Object neighbor : neighbors) {
//                double[] neighborPoint = (double[]) neighbor;
//                if (!Arrays.equals(neighborPoint, dataList.get(i))) {
//                    dist[index++] = euclideanDistance(dataList.get(i), neighborPoint);
//                    if (index >= k) break;
//                }
//            }
//
//            // 万一无法找满 k 个邻居，用 Double.MAX_VALUE 填充
//            while (index < k) {
