...
import com.szubd.I_niceMO_Observers;
...
//        System.out.println(rspRDD.LO(convertList()).collect());
        ...
        // LO
        JavaRDD<List<double[]>> center = listJavaRspRDD.LO(I_niceMOF(10, 10, 0.1));
        // 打印每个分区的内容
        //
...
//            WrappedArray<Integer> wrappedArray = (WrappedArray<Integer>) row.get(0);
    ...
    public static Function<JavaRDD<List<double[]>>, JavaRDD<List<double[]>>> I_niceMOGO(SparkSession spark) {
        return rdd -> {
            ...
            List<double[]> mergedData = I_niceMO_Observers.mergeDataGOSC(allData);
...
//                List<double[]> result = com.szubd.I_niceMO_Observers.generateExpandedData(dataVectors, 20);
                ...
                ArrayList<Sample> samples = convertToSamples(centersAll);
                ...
                int numberOfCluster = cluster.getCenterList().size();
                List<double[]> centermeage = new ArrayList<>();

                for (int i = 0; i < numberOfCluster; i++) {
                    centermeage.add(centersAll.get(cluster.getCenterList().get(i)));
                }
    ...
    public static ArrayList<Sample> convertToSamples(List<double[]> inputList) {
        ArrayList<Sample> samples = new ArrayList<>();

        for (double[] arr : inputList) {
            samples.add(new Sample(arr, "unknown"));
        }
